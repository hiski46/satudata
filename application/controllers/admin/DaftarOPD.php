...
class DaftarOPD extends CI_Controller
{
    ...
    public function hapus()
    {
        ...
        $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Anda berhasil menghapus akun admin</div>');
    ...
    public function gantipassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            // $this->load->view('admin/DaftarOPD',$data);
            $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Password Tidak Berhasil Diubah</div>');
            redirect('admin/daftaropd');
        } else {
            $password_lama = $this->input->post('pass1');
            $password_baru = $this->input->post('pwbaru');
            if (!password_verify($password_lama, $data['user']['password'])) {
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Pasword Yang Anda masukkan Salah</div>');
                redirect('admin/daftaropd');
            } else {
